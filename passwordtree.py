...
class PasswordTree:
    ...
    def __init__(self, num_buckets, q, tsv_path):
        ...
        if os.path.exists(self._file_intervals) and os.path.exists(self._file_interval_sizes):
            ...
            self.pw_data = pd.read_csv(tsv_path, sep='\t', header=None, names=['id', 'pw', 'freq', 'ng-prob', 'hash'],
                                       engine='python', error_bad_lines=False, quoting=csv.QUOTE_NONE).set_index('id')
    ...
    def __create_new_tree__(self):
        ...
        self._interval_size_df = pd.DataFrame(
            data={'id': ids_arr, 'pw': pws, 'size': interval_sizes, 'weight': weights})
    ...
    def get_bucket(self, bucket_num, limit=None):
        ...
        for index in self._tree.all_overlaps_both(starts, ends, np.array([0]))[1]:
            bucket_list += [
                (float(self._interval_size_df.loc[index, 'weight']), str(self._interval_size_df.loc[index, 'pw']))]
        if limit == None:
