...
def site_policy_baseline_experiment():
    ...
    get_targeted_guesses(site_policy_user_mul_pw_file, comp_df, site_policy_predictions_files, baseline=True,
                         site_policy=True)
...
def sample_baseline_pws(num_trials, comp_sample_file, uncomp_sample_file):
    ...
    if comp_sample_file != '':
        ...
        comp_df = pd.DataFrame(
            data={'user': users, 'pw': pws, 'rank': [-1] * num_trials, 'target_size': [0] * num_trials, \
...
def sample_user_pws(comp_sample_file, uncomp_sample_file, num_trials, hpb):
    ...
    if comp_sample_file != '':
        ...
        comp_df = pd.DataFrame(data={'bucket': buckets, 'user': users, 'pw': pws, 'known': [[]] * num_trials, \
                                     'target_guesses': [[]] * num_trials, 'hist_guesses': hist_guesses,
                                     'bucket_size': sizes})
        if not hpb:
...
def sample_corr_user_pws(corr_sample_file, num_trials, hpb):
    ...
    df = pd.DataFrame(data={'user': users, 'pw1': pw1s, 'pw2': pw2s, 'b1': b1s, 'b2': b2s, 'known1': [[]] * num_trials, \
                            'known2': [[]] * num_trials, 'target1': [[]] * num_trials, 'target2': [[]] * num_trials,
...
def get_targeted_guesses(user_pw_file, df, pw_files, corr=False, hpb=False, baseline=False, site_policy=False):
    ...
    with open(user_pw_file, 'r') as readfile:
        for line in readfile:
            ...
            pws = sorted(pd.Series(pw_list).drop_duplicates().tolist(),
                         key=lambda word: (-counts[word], -get_prob(word)))
            for index, row in df[df['user'] == user].iterrows():
                if corr and hpb:
                    df.at[index, 'known1'] = pd.Series(
                        list(get_predictions_hibp(pws, row['b1']))).drop_duplicates().tolist()
                    df.at[index, 'known2'] = pd.Series(
                        list(get_predictions_hibp(pws, row['b2']))).drop_duplicates().tolist()
                elif corr:
                    ...
                    df.at[index, 'known'] = pd.Series(
                        list(get_predictions_hibp(pws, row['bucket']))).drop_duplicates().tolist()
                elif baseline:
                    ...
                    if corr:
                        ...
                        if hpb:
                            df.at[row.name, 'target1'] = pd.Series( \
                                df.at[row.name, 'target1'] + list(
                                    get_predictions_hibp(pred_passwords, b1))).drop_duplicates().tolist()
                            df.at[row.name, 'target2'] = pd.Series( \
                                df.at[row.name, 'target2'] + list(
                                    get_predictions_hibp(pred_passwords, b2))).drop_duplicates().tolist()
                        else:
                            df.at[row.name, 'target1'] = pd.Series( \
                                df.at[row.name, 'target1'] + list(
                                    get_predictions(pred_passwords, b1))).drop_duplicates().tolist()
                            df.at[row.name, 'target2'] = pd.Series( \
                                df.at[row.name, 'target2'] + list(
                                    get_predictions(pred_passwords, b2))).drop_duplicates().tolist()
                    elif hpb:
                        bucket = row['bucket']
                        _id = row['user']
                        ...
                        df.at[bucket, 'target_guesses'] = pd.Series( \
                            df.at[bucket, 'target_guesses'] + list(
                                get_predictions(pred_passwords, bucket))).drop_duplicates().tolist()
